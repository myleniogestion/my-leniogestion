...
export class EntradaController{
    constructor(private readonly entradaService:EntradaService=new EntradaService()){
    ...
    async updateEntrada(req: Request, res: Response){
        const {ID} = req.params;
        try {
            const data = await this.entradaService.updateEntrada(parseInt(ID), req.body);
            res.status(200).json(data);
        } catch (e:any) {
            res.status(500).json({"error":e.message});            
        }
    ...
    async getAllEntradasbyProveedor(req:Request,res:Response){
        ...
        try {
            ...
            const data=await this.entradaService.getAllEntradasbyProveedor(parseInt(id_proveedor));
    ...
    async filtrarEntradas(req:Request,res:Response){
        ...
        try {
            const data= await this.entradaService.filtrarEntradas(nombre_proveedor,nombre_producto,costo_liminf,costo_limsup,fecha_liminf,fecha_limsup);
            (data)?res.status(200).json(data):res.status(404).json("No encontrado")
        }
    catch (error:any) {
            res.status(500).json({"error":error.message})
    ...
    async EntradasbyProducto(req:Request,res:Response){
        ...
        try {
            const data=await this.entradaService.EntradasbyProducto(parseInt(id_producto));
