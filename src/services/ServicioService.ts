...
export class ServicioService extends BaseService<Servicio> {
 ...
 async createServicio(body: ServicioDto): Promise<Servicio>{
   ...
   async updateServicio(id: number, infoUpdate: ServicioDto): Promise<UpdateResult>{
    return (await this.execRepository).update(id, infoUpdate);
    }
    async GananciaTotalServicio():Promise<number>{
        const ganancia_total=await this.findAllServicios()
       return ganancia_total.reduce((acumulador,servicio:Servicio)=>acumulador+servicio.precio,0)
    }
    async getServiciosbyTipo_servicio(id_tipo_servicio:number){
        ...
        .leftJoinAndSelect("Servicio.tienda","tienda")
        .leftJoinAndSelect("Servicio.diario", "diario")
        .where("tipo_servicio.id_tipo_servicio=:id_tipo_servicio",{id_tipo_servicio})
        .getMany()
    }
    async filtrarServicio(nombre_cliente:string|null,precio_liminf:number|null,precio_limsup:number|null,fecha_liminf:Date|null,fecha_limsup:Date|null,id_tipo_servicio:number|null,id_tienda:number|null,nombre_producto:string|null){
        ...
        if(nombre_cliente){
            ...
            servicios= servicios.filter((servicio:Servicio)=>normalizeString(servicio.cliente.nombre).toLowerCase().includes(normalizedNombre))    
        }
        if(precio_liminf){
        ...
        if(id_tipo_servicio){
            ...
            servicios=servicios.filter((servicio:Servicio)=>servicio.tipo_servicio.id_tipo_servicio==id_tipo_servicio);
        }
        if(id_tienda){
    ...
    async filtrarServicioJT(nombre_cliente:string|null,precio_liminf:number|null,precio_limsup:number|null,fecha_liminf:Date|null,fecha_limsup:Date|null,id_tipo_servicio:number,id_tienda:number|null){
        let servicios:Servicio[];
        const normalizeString = (str: string) => str.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
        if(fecha_liminf&&fecha_limsup==null){
            servicios=await(await this.execRepository)
            ...
            .leftJoinAndSelect("servicio.tipo_servicio","ts")
            .leftJoinAndSelect("servicio.diario", "diario")
            .where("servicio.id_tipo_servicio=:id_tipo_servicio",{id_tipo_servicio})
            .getMany()
        }
        if(nombre_cliente){
            const normalizedNombre = normalizeString(nombre_cliente);
