...
export class SalidaService extends BaseService<Salida> {
    ...
    async findAllSalidas():Promise<Salida[]> {
        return (await this.execRepository).find({relations:["producto","tienda_origen","tienda_destino","usuario"]});
    }
 ...
 async createSalida(body: SalidaDto): Promise<Salida>{
   ...
   async updateSalida(id: number, infoUpdate: SalidaDto): Promise<UpdateResult>{
    ...
    async filtrarSalida(nombre_usuario:string|null,nombre_producto:string|null,cantidad:number|null,fechaliminf:Date|null,fechalimsup:Date|null,id_tienda_origen:number|null,id_tienda_destino:number|null){
        let salidas:Salida[]=[]
        ...
        if(fechaliminf && fechalimsup){
            ...
            .leftJoinAndSelect("salida.usuario","u")
            .leftJoinAndSelect("salida.producto","p")
            .where("salida.fecha>=:fechaliminf and salida.fecha<=:fechalimsup",{fechaliminf,fechalimsup})
            .getMany();
            console.log(salidas);
        ...
        .getMany();
            ...
            salidas=await(await this.execRepository).find({relations:["producto","tienda_origen","tienda_destino","usuario"]})
            }
    
        if(nombre_usuario){
            const normalizednombre_usuario=  normalizeString(nombre_usuario);
            salidas=salidas.filter((salida:Salida)=>normalizeString(salida.usuario.nombre).includes(normalizednombre_usuario));
        }
        if(nombre_producto){
            const normalizedNombre_producto=  normalizeString(nombre_producto);
            salidas=salidas.filter((salida:Salida)=>normalizeString(salida.producto.nombre).includes(normalizedNombre_producto));
        }
        if(cantidad)
            salidas=salidas.filter((salida:Salida)=>salida.cantidad===cantidad)
       ...
       async    filtrarSalidasJT(nombre_usuario:string|null,nombre_producto:string|null,cantidad:number|null,fechaliminf:string|null,fechalimsup:string,id_tienda:number,id_tienda_origen:number|null,id_tienda_destino:number|null){
      ...
      /*  let  salidas:Salida[]=await(await this.execRepository)
        ...
        if(fechaliminf!=null&&fechalimsup==null){
        salidas=await(await this.execRepository)
        .createQueryBuilder("salida")
        .leftJoinAndSelect("salida.tienda_origen","to")
        .leftJoinAndSelect("salida.tienda_destino","td")
       ...
       console.log("-->"+salidas[0].fecha)
        if(nombre_usuario){
            ...
            const normalizednombre_usuario=  normalizeString(nombre_usuario);
            salidas=salidas.filter((salida:Salida)=>normalizeString(salida.usuario.nombre_usuario).includes(normalizednombre_usuario));
        }
        if(nombre_producto){
            console.log("usuario-->"+salidas[0].fecha)
            const normalizedNombre_producto=  normalizeString(nombre_producto);
        ...
        if(id_tienda_origen){
            salidas=salidas.filter((salida:Salida)=> salida.tienda_origen.id_tienda==id_tienda_origen)
        }
        if(id_tienda_destino){
            salidas=salidas.filter((salida:Salida)=> salida.tienda_destino.id_tienda==id_tienda_destino)
        }
